...
# al_haqq_dashboard.py — Unified Command Hub (v2.8)
...
st.set_page_config(
    page_title="Al-Haqq Protocol TCG — Command Hub",
    page_icon="⚖️",
    layout="wide"
)

st.title("⚖️ Al-Haqq Protocol TCG — Unified Command Hub")
...
with tab4:
    st.subheader("Rantai Ledger Kriptografis Al-Haqq (Blockchain)")
    ledger = AlHaqqLedger()
    if st.button("Verifikasi Integritas Rantai"):
        is_valid = ledger.verify_integrity()
        if is_valid:
            st.success("Integritas Rantai: VALID (Mizan Mutlak Terjaga)")
        else:
            st.error("Integritas Rantai: ANOMALI TERDETEKSI")
    
    st.write("Daftar Blok Ledger Saat Ini:")
